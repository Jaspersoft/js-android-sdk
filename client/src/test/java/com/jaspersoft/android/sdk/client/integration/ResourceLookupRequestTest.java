...
public class ResourceLookupRequestTest extends ParametrizedTest {
    ...
    @Test
    public void shouldRequestForResourceLookup() throws Exception {
        JsRestClient jsRestClient = getJsRestClient();
        ResourceLookupSearchCriteria searchCriteria = getFactoryGirl().createSearchCriteria();
        GetResourceLookupsRequest request = new GetResourceLookupsRequest(jsRestClient, searchCriteria);
        ResourceLookupsList response = request.loadDataFromNetwork();
        assertThat(response.getResourceLookups(), is(not(empty())));
    }
}
