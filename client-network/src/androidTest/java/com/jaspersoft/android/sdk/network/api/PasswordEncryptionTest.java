package com.jaspersoft.android.sdk.network.api;

import android.test.AndroidTestCase;

/**
 * @author Tom Koptel
 * @since 2.0
 */
public class PasswordEncryptionTest extends AndroidTestCase {
    private final static String PUBLIC_EXPONENT = "010001";
    private final static String MODULUS = "";
    private final static String RESULT = "";

    public void testGenerateKey() throws Exception {
        PasswordEncryption keyGenerator = new PasswordEncryption(MODULUS, PUBLIC_EXPONENT);
        String encryptedPass = keyGenerator.encrypt("superuser");
        assertTrue(RESULT.equals(encryptedPass));
    }
}
