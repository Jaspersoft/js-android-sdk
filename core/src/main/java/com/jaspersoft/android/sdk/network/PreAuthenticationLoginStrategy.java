 ...
 * @since 2.3
 */
class PreAuthenticationLoginStrategy {
    @NotNull
    ...
    @TestOnly
    PreAuthenticationLoginStrategy(@NotNull PreAuthenticationServiceFactory preAuthenticationServiceFactory,
    ...
    void apply(PreAuthenticationCredentials credentials) throws IOException, HttpException {
        authenticationLifecycle.beforeSessionReload();
        authService().authenticate(credentials);
        authenticationLifecycle.afterSessionReload();
    }

    private PreAuthenticationLoginService authService() {
